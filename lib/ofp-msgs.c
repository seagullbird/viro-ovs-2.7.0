               ...
               const struct ofp_header *oh, size_t length)
{
    ...
    if (hdrs->type == OFPT_VENDOR) {
        ...
        if (hdrs->stat == OFPST_VENDOR) {
            ...
            } else if (hdrs->vendor == VIRO_VENDOR_ID) {
                const struct viro_header *vh;

                if (length < sizeof *vh) {
                    return OFPERR_OFPBRC_BAD_LEN;
                }
                vh = (const struct viro_header*) oh;
                hdrs->subtype = ntohl(vh->subtype);
            } else {
                log_bad_vendor(hdrs->vendor);
                return OFPERR_OFPBRC_BAD_VENDOR;
            }
             ...
             size_t extra_tailroom, struct ofpbuf *buf)
{
    ...
    if (hdrs->type == OFPT_VENDOR) {
        ovs_assert(hdrs->vendor == NX_VENDOR_ID || hdrs->vendor == VIRO_VENDOR_ID);
        ...
        if (hdrs->vendor == NX_VENDOR_ID) {
            ...
            struct viro_header *vh = buf->header;
            vh->vendor = htonl(hdrs->vendor);
            vh->subtype = htonl(hdrs->subtype);
        }
