...
flow_extract(struct dp_packet *packet, struct flow *flow)
{
    ...
    viro_miniflow_extract(packet, &m.mf, flow);
 ...
 * FLOW_U64S * 8 bytes. */
void
viro_miniflow_extract(struct dp_packet *packet, struct miniflow *dst, struct flow *flow)
{
    ...
    if (OVS_LIKELY(dl_type == htons(ETH_TYPE_VIRO)) && flow != NULL) {
        ...
        ovs_be32 src_sw = flow->dl_src.be16[0];
        src_sw = src_sw << 16;
        src_sw += flow->dl_src.be16[1];
        ovs_be16 src_host = flow->dl_src.be16[2];
        miniflow_push_be32(mf, viro_src_sw, src_sw);
        miniflow_push_be16(mf, viro_src_host, src_host);


        ovs_be32 dst_sw = flow->dl_dst.be16[0];
        dst_sw = dst_sw << 16;
        dst_sw += flow->dl_dst.be16[1];
        ovs_be16 dst_host = flow->dl_dst.be16[2];
        miniflow_push_be32(mf, viro_dst_sw, dst_sw);
        miniflow_push_be16(mf, viro_dst_host, dst_host);
    }
