...
mf_is_all_wild(const struct mf_field *mf, const struct flow_wildcards *wc)
{
    ...
    case MFF_VIRO_DST_SW:
        return !wc->masks.viro_dst_sw;
    case MFF_VIRO_DST_HOST:
        return !wc->masks.viro_dst_host;
    case MFF_VIRO_SRC_SW:
        return !wc->masks.viro_src_sw;
    case MFF_VIRO_SRC_HOST:
        return !wc->masks.viro_src_host;
    case MFF_VIRO_FD_SW:
        return !wc->masks.viro_fd_sw;
    case MFF_VIRO_FD_HOST:
        return !wc->masks.viro_fd_host;
             ...
             union mf_value *value)
{
    ...
    case MFF_VIRO_DST_SW:
        value->be32 = flow->viro_dst_sw;
        break;
    case MFF_VIRO_DST_HOST:
        value->be16 = flow->viro_dst_host;
        break;
    case MFF_VIRO_SRC_SW:
        value->be32 = flow->viro_src_sw;
        break;
    case MFF_VIRO_SRC_HOST:
        value->be16 = flow->viro_src_host;
        break;
    case MFF_VIRO_FD_SW:
        value->be32 = flow->viro_fd_sw;
        break;
    case MFF_VIRO_FD_HOST:
        value->be16 = flow->viro_fd_host;
        break;
             ...
             const union mf_value *value, struct match *match, char **err_str)
{
    ...
    case MFF_VIRO_DST_SW:
        match_set_viro_dst_sw(match, value->be32);
        break;
    case MFF_VIRO_DST_HOST:
        match_set_viro_dst_host(match, value->be16);
        break;
    case MFF_VIRO_SRC_SW:
        match_set_viro_src_sw(match, value->be32);
        break;
    case MFF_VIRO_SRC_HOST:
        match_set_viro_src_host(match, value->be16);
        break;
    case MFF_VIRO_FD_SW:
        match_set_viro_fd_sw(match, value->be32);
        break;
    case MFF_VIRO_FD_HOST:
        match_set_viro_fd_host(match, value->be16);
        break;
                  ...
                  const union mf_value *value, struct flow *flow)
{
    ...
    case MFF_VIRO_DST_SW:
        flow->viro_dst_sw = value->be32;
        break;
    case MFF_VIRO_DST_HOST:
        flow->viro_dst_host = value->be16;
        break;
    case MFF_VIRO_SRC_SW:
        flow->viro_src_sw = value->be32;
        break;
    case MFF_VIRO_SRC_HOST:
        flow->viro_src_host = value->be16;
        break;
    case MFF_VIRO_FD_SW:
        flow->viro_fd_sw = value->be32;
        break;
    case MFF_VIRO_FD_HOST:
        flow->viro_fd_host = value->be16;
        break;
...
mf_set_wild(const struct mf_field *mf, struct match *match, char **err_str)
{
    ...
    case MFF_VIRO_DST_SW:
        match_set_viro_dst_sw_masked(match, htonl(0), htonl(0));
        break;
    case MFF_VIRO_DST_HOST:
        match->flow.viro_dst_host = htons(0);
        match->wc.masks.viro_dst_host = htons(0);
        break;
    case MFF_VIRO_SRC_SW:
        match_set_viro_src_sw_masked(match, htonl(0), htonl(0));
        break;
    case MFF_VIRO_SRC_HOST:
        match->flow.viro_src_host = htons(0);
        match->wc.masks.viro_src_host = htons(0);
        break;
    case MFF_VIRO_FD_SW:
        match_set_viro_fd_sw_masked(match, htonl(0), htonl(0));
        break;
    case MFF_VIRO_FD_HOST:
        match->flow.viro_fd_host = htons(0);
        match->wc.masks.viro_fd_host = htons(0);
        break;
       ...
       struct match *match, char **err_str)
{
    ...
    case MFF_VIRO_DST_SW:
        match_set_viro_dst_sw_masked(match, value->be32, mask->be32);
        break;
    case MFF_VIRO_SRC_SW:
        match_set_viro_src_sw_masked(match, value->be32, mask->be32);
        break;
    case MFF_VIRO_FD_SW:
        match_set_viro_fd_sw_masked(match, value->be32, mask->be32);
        break;
    case MFF_VIRO_DST_HOST:
    case MFF_VIRO_SRC_HOST:
    case MFF_VIRO_FD_HOST:
