...
void
match_set_viro_dst_sw_masked(struct match *match, ovs_be32 sw, ovs_be32 mask)
{
    match->flow.viro_dst_sw = sw & mask;
    match->wc.masks.viro_dst_sw = mask;
}

void
match_set_viro_dst_sw(struct match *match, ovs_be32 sw)
{
    match_set_viro_dst_sw_masked(match, sw, htonl(UINT32_MAX));
}

void
match_set_viro_src_sw_masked(struct match *match, ovs_be32 sw, ovs_be32 mask)
{
    match->flow.viro_src_sw = sw & mask;
    match->wc.masks.viro_src_sw = mask;
}

void
match_set_viro_src_sw(struct match *match, ovs_be32 sw)
{
    match_set_viro_src_sw_masked(match, sw, htonl(UINT32_MAX));
}

void
match_set_viro_fd_sw_masked(struct match *match, ovs_be32 sw, ovs_be32 mask)
{
    match->flow.viro_fd_sw = sw & mask;
    match->wc.masks.viro_fd_sw = mask;
}

void
match_set_viro_fd_sw(struct match *match, ovs_be32 sw)
{
    match_set_viro_fd_sw_masked(match, sw, htonl(UINT32_MAX));
}

void
match_set_viro_dst_host(struct match *match, ovs_be16 host)
{
    match->flow.viro_dst_host = host;
    match->wc.masks.viro_dst_host = htons(UINT16_MAX);
}

void
match_set_viro_src_host(struct match *match, ovs_be16 host)
{
    match->flow.viro_src_host = host;
    match->wc.masks.viro_src_host = htons(UINT16_MAX);
}

void
match_set_viro_fd_host(struct match *match, ovs_be16 host)
{
    match->flow.viro_fd_host = host;
    match->wc.masks.viro_fd_host = htons(UINT16_MAX);
}
