...
/* ## ----------------- ## */
    ...
    /* OF1.0(12), OF1.1(33), OF1.2+(33) is deprecated (use Set-Field): struct
     *ofp_action_viro_push_fd. */
    OFPAT_RAW_VIRO_PUSH_FD,

    /* OF1.0(13), OF1.1(26), OF1.2+(26) is deprecated (use Set-Field): void. */
    OFPAT_RAW_VIRO_POP_FD,

    /* OF1.0(14), OF1.1(27), OF1.2+(27) is deprecated (use Set-Field): struct
     *ofp_action_viro_set_vid_sw. */
    OFPAT_RAW_VIRO_SET_VID_SW_FD,

    /* OF1.0(15), OF1.1(34), OF1.2+(34) is deprecated (use Set-Field): struct
     *ofp_action_viro_set_vid_sw. */
    OFPAT_RAW_VIRO_SET_VID_SW_SRC,

    /* OF1.0(16), OF1.1(29), OF1.2+(29) is deprecated (use Set-Field): struct
     *ofp_action_viro_set_vid_sw. */
    OFPAT_RAW_VIRO_SET_VID_SW_DST,

    /* OF1.0(17), OF1.1(30), OF1.2+(30) is deprecated (use Set-Field): struct
     *ofp_action_viro_set_vid_host. */
    OFPAT_RAW_VIRO_SET_VID_HOST_FD,

    /* OF1.0(18), OF1.1(31), OF1.2+(31) is deprecated (use Set-Field): struct
     *ofp_action_viro_set_vid_host. */
    OFPAT_RAW_VIRO_SET_VID_HOST_SRC,

    /* OF1.0(19), OF1.1(32), OF1.2+(32) is deprecated (use Set-Field): struct
     *ofp_action_viro_set_vid_host. */
    OFPAT_RAW_VIRO_SET_VID_HOST_DST,
...
/* Pop viro fd action. */
static enum ofperr
decode_OFPAT_RAW_VIRO_POP_FD(struct ofpbuf *out)
{
    ofpact_put_VIRO_POP_FD(out);
    return 0;
}

static void
encode_VIRO_POP_FD(const struct ofpact_null *null,
                   enum ofp_version ofp_version, struct ofpbuf *out)
{
    put_OFPAT_VIRO_POP_FD(out, ofp_version);
}

static char * OVS_WARN_UNUSED_RESULT
parse_VIRO_POP_FD(char *arg, struct ofpbuf *ofpacts,
                enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    ofpact_put_VIRO_POP_FD(ofpacts);
    return NULL;
}

static void
format_VIRO_POP_FD(const struct ofpact_null *a OVS_UNUSED, struct ds *s)
{
    ds_put_cstr(s, "viro_pop_fd");
}
/*Push viro fd action. */

/* Action structure for OFPAT_RAW10_VIRO_PUSHFD*/
struct ofp_action_viro_push_fd {
    ovs_be16 type;                  /* Type. */
    ovs_be16 len;
    ovs_be32 fd_sw;
    ovs_be16 fd_host;
...
OFP_ASSERT(sizeof(struct ofp_action_viro_push_fd) == 16);


static enum ofperr
decode_OFPAT_RAW_VIRO_PUSH_FD(const struct ofp_action_viro_push_fd *vpd,
                                enum ofp_version ofp_version OVS_UNUSED,
                                struct ofpbuf *out)
{
    struct ofpact_viro_push_fd *viropd;

    viropd = ofpact_put_VIRO_PUSH_FD(out);
    viropd->fd_host = ntohs(vpd->fd_host);
    viropd->fd_sw = ntohl(vpd->fd_sw);
    return 0;
}

static void
encode_VIRO_PUSH_FD(const struct ofpact_viro_push_fd *viro_push_fd,
                   enum ofp_version ofp_version , struct ofpbuf *out)
{
        struct ofp_action_viro_push_fd *vpd;
        vpd = put_OFPAT_VIRO_PUSH_FD(out, ofp_version);
        vpd->fd_host = htons(viro_push_fd->fd_host);
        vpd->fd_sw = htonl(viro_push_fd->fd_sw);
}

static char * OVS_WARN_UNUSED_RESULT
parse_VIRO_PUSH_FD(char *arg, struct ofpbuf *ofpacts,
                enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    struct ofpact_viro_push_fd *viro_push_fd;
    char *key, *value;
    char *error;

    viro_push_fd = ofpact_put_VIRO_PUSH_FD(ofpacts);

    ofputil_parse_key_value(&arg, &key, &value);
    uint32_t valuep32;
    error = str_to_u32(value, &valuep32);
    if (!error)
        viro_push_fd->fd_sw = htonl(valuep32);
    else
        return error;

    ofputil_parse_key_value(&arg, &key, &value);
    uint16_t valuep16;
    error = str_to_u16(value, key, &valuep16);
    if(!error)
        viro_push_fd->fd_host = htons(valuep16);
    return error;
}

static void
format_VIRO_PUSH_FD(const struct ofpact_viro_push_fd *a, struct ds *s)
{
    ds_put_format(s, "viro_push_fd(sw=%#"PRIx32",host=%#"PRIx16")",ntohl(a->fd_sw), ntohs(a->fd_host));
}

/* Set viro vid sw fd actions. */
struct ofp_action_viro_set_vid_sw{
    ovs_be16 type;                  /* Type. */
    ovs_be16 len;
    ovs_be32 sw;
};
OFP_ASSERT(sizeof(struct ofp_action_viro_set_vid_sw) == 8);

static enum ofperr
decode_OFPAT_RAW_VIRO_SET_VID_SW_FD(const struct ofp_action_viro_set_vid_sw *vsvs,
                                    enum ofp_version ofp_version OVS_UNUSED,
                                    struct ofpbuf *out)
{
    struct ofpact_viro_set_vid_sw *virosw;

    virosw = ofpact_put_VIRO_SET_VID_SW_FD(out);
    virosw->sw = ntohl(vsvs->sw);
    return 0;
}

static void
encode_VIRO_SET_VID_SW_FD(const struct ofpact_viro_set_vid_sw *viro_set_vid_sw,
                   enum ofp_version ofp_version , struct ofpbuf *out)
{
        struct ofp_action_viro_set_vid_sw *vsvs;
        vsvs = put_OFPAT_VIRO_SET_VID_SW_FD(out, ofp_version);
        vsvs->sw = htonl(viro_set_vid_sw->sw);
}

static char * OVS_WARN_UNUSED_RESULT
parse_VIRO_SET_VID_SW_FD(char *arg, struct ofpbuf *ofpacts,
                enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    char *error;
    uint32_t valuep;
    error = str_to_u32(arg, &valuep);
    if(!error)
        ofpact_put_VIRO_SET_VID_SW_FD(ofpacts)->sw = htonl(valuep);
    return error;
}

static void
format_VIRO_SET_VID_SW_FD(const struct ofpact_viro_set_vid_sw *a, struct ds *s)
{
    ds_put_format(s, "viro_set_vid_sw_fd:%#"PRIx32, ntohl(a->sw));
}


/* Set viro vid sw src action. */
static enum ofperr
decode_OFPAT_RAW_VIRO_SET_VID_SW_SRC(const struct ofp_action_viro_set_vid_sw *vsvs,
                                    enum ofp_version ofp_version OVS_UNUSED,
                                    struct ofpbuf *out)
{
    struct ofpact_viro_set_vid_sw *virosw;

    virosw = ofpact_put_VIRO_SET_VID_SW_SRC(out);
    virosw->sw = ntohl(vsvs->sw);
    return 0;
}

static void
encode_VIRO_SET_VID_SW_SRC(const struct ofpact_viro_set_vid_sw *viro_set_vid_sw,
                   enum ofp_version ofp_version , struct ofpbuf *out)
{
        struct ofp_action_viro_set_vid_sw *vsvs;
        vsvs = put_OFPAT_VIRO_SET_VID_SW_SRC(out, ofp_version);
        vsvs->sw = htonl(viro_set_vid_sw->sw);
}

static char * OVS_WARN_UNUSED_RESULT
parse_VIRO_SET_VID_SW_SRC(char *arg, struct ofpbuf *ofpacts,
                enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    char *error;
    uint32_t valuep;
    error = str_to_u32(arg, &valuep);
    if(!error)
        ofpact_put_VIRO_SET_VID_SW_SRC(ofpacts)->sw = htonl(valuep);
    return error;
}

static void
format_VIRO_SET_VID_SW_SRC(const struct ofpact_viro_set_vid_sw *a, struct ds *s)
{
   ds_put_format(s, "viro_set_vid_sw_src:%#"PRIx32, ntohl(a->sw));
}


/* Set viro vid sw dst action.*/
static enum ofperr
decode_OFPAT_RAW_VIRO_SET_VID_SW_DST(const struct ofp_action_viro_set_vid_sw *vsvs,
                                    enum ofp_version ofp_version OVS_UNUSED,
                                    struct ofpbuf *out)
{
    struct ofpact_viro_set_vid_sw *virosw;

    virosw = ofpact_put_VIRO_SET_VID_SW_DST(out);
    virosw->sw = ntohl(vsvs->sw);
    return 0;
}

static void
encode_VIRO_SET_VID_SW_DST(const struct ofpact_viro_set_vid_sw *viro_set_vid_sw,
                   enum ofp_version ofp_version , struct ofpbuf *out)
{
        struct ofp_action_viro_set_vid_sw *vsvs;
        vsvs = put_OFPAT_VIRO_SET_VID_SW_DST(out, ofp_version);
        vsvs->sw = htonl(viro_set_vid_sw->sw);
}

static char * OVS_WARN_UNUSED_RESULT
parse_VIRO_SET_VID_SW_DST(char *arg, struct ofpbuf *ofpacts,
                enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    char *error;
    uint32_t valuep;
    error = str_to_u32(arg, &valuep);
    if(!error)
        ofpact_put_VIRO_SET_VID_SW_DST(ofpacts)->sw = htonl(valuep);
    return error;
}

static void
format_VIRO_SET_VID_SW_DST(const struct ofpact_viro_set_vid_sw *a, struct ds *s)
{
   ds_put_format(s, "viro_set_vid_sw_dst:%#"PRIx32, ntohl(a->sw));
}
/* Action struct for VIRO_SETVID_HOST*/
struct ofp_action_viro_set_vid_host{
    ovs_be16 type;                  /* Type. */
    ovs_be16 len;
    ovs_be16 host;
    uint8_t pad[2];
};
OFP_ASSERT(sizeof(struct ofp_action_viro_set_vid_host) == 8);


/*Set viro vid host fd action.*/
static enum ofperr
decode_OFPAT_RAW_VIRO_SET_VID_HOST_FD(const struct ofp_action_viro_set_vid_host *vsvh,
                                    enum ofp_version ofp_version OVS_UNUSED,
                                    struct ofpbuf *out)
{
    struct ofpact_viro_set_vid_host *viroht;

    viroht = ofpact_put_VIRO_SET_VID_HOST_FD(out);
    viroht->host = ntohs(vsvh->host);
    return 0;
}

static void
encode_VIRO_SET_VID_HOST_FD(const struct ofpact_viro_set_vid_host *viroht,
                   enum ofp_version ofp_version , struct ofpbuf *out)
{
        struct ofp_action_viro_set_vid_host *vsvh;
        vsvh = put_OFPAT_VIRO_SET_VID_HOST_FD(out, ofp_version);
        vsvh->host = htonl(viroht->host);
}

static char * OVS_WARN_UNUSED_RESULT
parse_VIRO_SET_VID_HOST_FD(char *arg, struct ofpbuf *ofpacts,
                enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    char *error;
    uint32_t valuep;
    error = str_to_u32(arg, &valuep);
    if(!error)
        ofpact_put_VIRO_SET_VID_HOST_FD(ofpacts)->host = htons(valuep);
    return error;
}


static void
format_VIRO_SET_VID_HOST_FD(const struct ofpact_viro_set_vid_host *a, struct ds *s)
{
   ds_put_format(s, "viro_set_vid_host_fd:%#"PRIx16, ntohs(a->host));
}

/*Set viro vid host src action.*/
static enum ofperr
decode_OFPAT_RAW_VIRO_SET_VID_HOST_SRC(const struct ofp_action_viro_set_vid_host *vsvh,
                                    enum ofp_version ofp_version OVS_UNUSED,
                                    struct ofpbuf *out)
{
    struct ofpact_viro_set_vid_host *viroht;

    viroht = ofpact_put_VIRO_SET_VID_HOST_SRC(out);
    viroht->host = ntohs(vsvh->host);
    return 0;
}

static void
encode_VIRO_SET_VID_HOST_SRC(const struct ofpact_viro_set_vid_host *viroht,
                   enum ofp_version ofp_version , struct ofpbuf *out)
{
        struct ofp_action_viro_set_vid_host *vsvh;
        vsvh = put_OFPAT_VIRO_SET_VID_HOST_SRC(out, ofp_version);
        vsvh->host = htonl(viroht->host);
}

static char * OVS_WARN_UNUSED_RESULT
parse_VIRO_SET_VID_HOST_SRC(char *arg, struct ofpbuf *ofpacts,
                enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    char *error;
    uint32_t valuep;
    error = str_to_u32(arg, &valuep);
    if(!error)
        ofpact_put_VIRO_SET_VID_HOST_SRC(ofpacts)->host = htons(valuep);
    return error;
}

static void
format_VIRO_SET_VID_HOST_SRC(const struct ofpact_viro_set_vid_host *a, struct ds *s)
{
   ds_put_format(s, "viro_set_vid_host_src:%#"PRIx16, ntohs(a->host));
}


/*Set viro vid host dst action.*/
static enum ofperr
decode_OFPAT_RAW_VIRO_SET_VID_HOST_DST(const struct ofp_action_viro_set_vid_host *vsvh,
                                    enum ofp_version ofp_version OVS_UNUSED,
                                    struct ofpbuf *out)
{
    struct ofpact_viro_set_vid_host *viroht;

    viroht = ofpact_put_VIRO_SET_VID_HOST_DST(out);
    viroht->host = ntohs(vsvh->host);
    return 0;
}

static void
encode_VIRO_SET_VID_HOST_DST(const struct ofpact_viro_set_vid_host *viroht,
                   enum ofp_version ofp_version , struct ofpbuf *out)
{
        struct ofp_action_viro_set_vid_host *vsvh;
        vsvh = put_OFPAT_VIRO_SET_VID_HOST_DST(out, ofp_version);
        vsvh->host = htonl(viroht->host);
}

static char * OVS_WARN_UNUSED_RESULT
parse_VIRO_SET_VID_HOST_DST(char *arg, struct ofpbuf *ofpacts,
                enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    char *error;
    uint32_t valuep;
    error = str_to_u32(arg, &valuep);
    if(!error)
        ofpact_put_VIRO_SET_VID_HOST_DST(ofpacts)->host = htons(valuep);
    return error;
}

static void
format_VIRO_SET_VID_HOST_DST(const struct ofpact_viro_set_vid_host *a, struct ds *s)
{
   ds_put_format(s, "viro_set_vid_host_dst:%#"PRIx16, ntohs(a->host));
}
...
ofpact_is_allowed_in_actions_set(const struct ofpact *a)
{
    ...
    case OFPACT_VIRO_POP_FD:
        return true;
    //I don't know how to put them to the right places???
...
get_ofpact_map(enum ofp_version version)
{
    ...
    static const struct ofpact_map of10[] = {
        ...
        { OFPACT_ENQUEUE, 11 },
        { OFPACT_VIRO_PUSH_FD, 12 },
        { OFPACT_VIRO_POP_FD, 13 },
        { OFPACT_VIRO_SET_VID_SW_FD, 14 },
        { OFPACT_VIRO_SET_VID_SW_SRC, 15 },
        { OFPACT_VIRO_SET_VID_SW_DST, 16 },
        { OFPACT_VIRO_SET_VID_HOST_FD, 17 },
        { OFPACT_VIRO_SET_VID_HOST_SRC, 18 },
        { OFPACT_VIRO_SET_VID_HOST_DST, 19 },
        { 0, -1 },
    };
    ...
    static const struct ofpact_map of11[] = {
        ...
        { OFPACT_DEC_TTL, 24 },
        { OFPACT_VIRO_POP_FD, 26 },
        { OFPACT_VIRO_SET_VID_SW_FD, 27 },
        { OFPACT_VIRO_SET_VID_SW_DST, 29 },
        { OFPACT_VIRO_SET_VID_HOST_FD, 30 },
        { OFPACT_VIRO_SET_VID_HOST_SRC, 31 },
        { OFPACT_VIRO_SET_VID_HOST_DST, 32 },
        { OFPACT_VIRO_PUSH_FD, 33 },
        { OFPACT_VIRO_SET_VID_SW_SRC, 34 },
        { 0, -1 },
    };
