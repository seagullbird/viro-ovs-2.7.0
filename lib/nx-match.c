           ...
           ovs_be64 cookie, ovs_be64 cookie_mask)
{
    ...
    if (flow->dl_type == htons(ETH_TYPE_VIRO)) {
        nxm_put_32m(b, MFF_VIRO_DST_SW, 0x01, flow->viro_dst_sw, match->wc.masks.viro_dst_sw);
        nxm_put_16m(b, MFF_VIRO_DST_HOST, 0x01, flow->viro_dst_host, match->wc.masks.viro_dst_host);

        nxm_put_32m(b, MFF_VIRO_SRC_SW, 0x01, flow->viro_src_sw, match->wc.masks.viro_src_sw);
        nxm_put_16m(b, MFF_VIRO_SRC_HOST, 0x01, flow->viro_src_host, match->wc.masks.viro_src_host);

        nxm_put_32m(b, MFF_VIRO_FD_SW, 0x01, flow->viro_fd_sw, match->wc.masks.viro_fd_sw);
        nxm_put_16m(b, MFF_VIRO_FD_HOST, 0x01, flow->viro_fd_host, match->wc.masks.viro_fd_host);
    }
