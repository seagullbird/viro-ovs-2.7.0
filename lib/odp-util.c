...
odp_action_len(uint16_t type)
{
    ...
    case OVS_ACTION_ATTR_VIRO_PUSH_FD: return sizeof(struct ovs_action_viro_push_fd);
    case OVS_ACTION_ATTR_VIRO_POP_FD: return 0;
...
ovs_key_attr_to_string(enum ovs_key_attr attr, char *namebuf, size_t bufsize)
{
    ...
    case OVS_KEY_ATTR_VIRO: return "viro";
...
format_odp_action(struct ds *ds, const struct nlattr *a)
{
    ...
    size_t size;
    const struct ovs_action_viro_push_fd *viro_fd;
    ...
    case OVS_ACTION_ATTR_VIRO_PUSH_FD:
        viro_fd = nl_attr_get(a);
        ds_put_format(ds, "push_viro_fd(sw=%#"PRIx32",host=%#"PRIx16")", ntohl(viro_fd->fd_sw), ntohs(viro_fd->fd_host));
        break;

    case OVS_ACTION_ATTR_VIRO_POP_FD:
        ds_put_cstr(ds, "pop_viro_fd");
        break;
                 ...
                 struct ofpbuf *actions)
{
...

    {
        ...
        {
        struct ovs_action_viro_push_fd push_fd;
        int n = -1;
        int sw, host;

        if (sscanf(s, "push_viro_fd(sw=%i,host=%i)%n", &sw, &host, &n) > 0 && n > 0) {
            push_fd.fd_sw = htonl(sw);
            push_fd.fd_host = htons(host);
            nl_msg_put_unspec(actions, OVS_ACTION_ATTR_VIRO_PUSH_FD, &push_fd, sizeof push_fd);
            return n;
        }
    }

    if (!strncmp(s, "pop_viro_fd", 11)) {
        nl_msg_put_flag(actions, OVS_ACTION_ATTR_VIRO_POP_FD);
        return 11;
    }
...
static const struct attr_len_tbl ovs_flow_key_attr_lens[OVS_KEY_ATTR_MAX + 1] = {
    ...
    [OVS_KEY_ATTR_VIRO] = { .len = sizeof(struct ovs_key_viro) },
};
                    ...
                    bool verbose)
{
    ...
    case OVS_KEY_ATTR_VIRO: {
        const struct ovs_key_viro *viro_key = nl_attr_get(a);
        ds_put_format(ds, "(src_sw=%#"PRIx32",src_host=%#"PRIx16
                    ",dst_sw=%#"PRIx32",dst_host=%#"PRIx16
                    ",fd_sw=%#"PRIx32",fd_host=%#"PRIx16")",
                    ntohl(viro_key->sw_src), ntohs(viro_key->host_src),
                    ntohl(viro_key->sw_dst), ntohs(viro_key->host_dst),
                    ntohl(viro_key->sw_fd), ntohs(viro_key->host_fd));
        break;
    }
                         ...
                         bool export_mask, struct ofpbuf *buf)
{
    ...
    if (flow->dl_type == htons(ETH_TYPE_VIRO)) {
        struct ovs_key_viro *viro_key;

        viro_key = nl_msg_put_unspec_uninit(buf, OVS_KEY_ATTR_VIRO, sizeof *viro_key);

        viro_key->sw_src = flow->viro_src_sw;
        viro_key->sw_dst = flow->viro_dst_sw;
        viro_key->sw_fd = flow->viro_fd_sw;
        viro_key->host_src = flow->viro_src_host;
        viro_key->host_dst = flow->viro_dst_host;
        viro_key->host_fd = flow->viro_fd_host;
                       ...
                       struct flow *flow, const struct flow *src_flow)
{
    ...
    if (src_flow->dl_type == htons(ETH_TYPE_VIRO)) {
        const struct ovs_key_viro *viro_key;

        viro_key = nl_attr_get(attrs[OVS_KEY_ATTR_VIRO]);
        flow->viro_src_sw = viro_key->sw_src;
        flow->viro_dst_sw = viro_key->sw_dst;
        flow->viro_fd_sw = viro_key->sw_fd;
        flow->viro_src_host = viro_key->host_src;
        flow->viro_dst_host = viro_key->host_dst;
        flow->viro_fd_host = viro_key->host_fd;
        return ODP_FIT_PERFECT;
    }
...
static void
commit_viro_fd_action(const struct flow *flow, struct flow *base,
                      struct ofpbuf *odp_actions)
{
    if (base->viro_fd_sw == flow->viro_fd_sw &&
        base->viro_fd_host == flow->viro_fd_host) {
        return;
    }

    if (base->viro_fd_sw != htonl(0)) {
        nl_msg_put_flag(odp_actions, OVS_ACTION_ATTR_VIRO_POP_FD);
    }

    if (flow->viro_fd_sw != htonl(0)) {
        struct ovs_action_viro_push_fd fd;

        fd.fd_sw = flow->viro_fd_sw;
        fd.fd_host = flow->viro_fd_host;
        nl_msg_put_unspec(odp_actions, OVS_ACTION_ATTR_VIRO_PUSH_FD, &fd, sizeof fd);
    }

    base->viro_fd_sw = flow->viro_fd_sw;
    base->viro_fd_host = flow->viro_fd_host;
}

static void
commit_set_viro_action(const struct flow *flow, struct flow *base,
                     struct ofpbuf *odp_actions)
{
    struct ovs_key_viro viro_key;

    if (base->viro_src_sw == flow->viro_src_sw &&
        base->viro_dst_sw == flow->viro_dst_sw &&
        base->viro_fd_sw == flow->viro_fd_sw &&
        base->viro_src_host == flow->viro_src_host &&
        base->viro_dst_host == flow->viro_dst_host &&
        base->viro_fd_host == flow->viro_fd_host) {
        return;
    }

    viro_key.sw_src = base->viro_src_sw = flow->viro_src_sw;
    viro_key.sw_dst = base->viro_dst_sw = flow->viro_dst_sw;
    viro_key.sw_fd = base->viro_fd_sw = flow->viro_fd_sw;

    viro_key.host_src = base->viro_src_host = flow->viro_src_host;
    viro_key.host_dst = base->viro_dst_host = flow->viro_dst_host;
    viro_key.host_fd = base->viro_fd_host = flow->viro_fd_host;


    commit_set_action(odp_actions, OVS_KEY_ATTR_VIRO, &viro_key, sizeof(viro_key));
}
                   ...
                   bool use_masked)
{
    ...
    commit_viro_fd_action(flow, base, odp_actions);
    commit_set_viro_action(flow, base, odp_actions);
