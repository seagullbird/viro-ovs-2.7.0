...
#define ETH_TYPE_VIRO 0x0802
#define VIRO_ETH_HDR_LEN 22
#define VIRO_HDR_LEN 8
#define VIRO_TYPE_LEN 2
#define VIRO_ADDR_LEN 6

void viro_push_fd(struct dp_packet *, ovs_be32 fd_sw, ovs_be16 fd_host);
void viro_pop_fd(struct dp_packet *);
void packet_set_viro(struct dp_packet *packet, ovs_be32 dst_sw, ovs_be16 dst_host, ovs_be32 src_sw, ovs_be16 src_host, ovs_be32 fd_sw, ovs_be16 fd_host);

struct viro_addr {
    ovs_be32 sw;
    ovs_be16 host;
} __attribute__((packed));
BUILD_ASSERT_DECL(VIRO_ADDR_LEN == sizeof(struct viro_addr));

struct viro_hdr {
    ovs_be32 fd_sw;
    ovs_be16 fd_host;
    ovs_be16 eth_next_type;
} __attribute__((packed));
BUILD_ASSERT_DECL(VIRO_HDR_LEN == sizeof(struct viro_hdr));

struct viro_eth_hdr {
    uint8_t eth_dst[ETH_ADDR_LEN];
    uint8_t eth_src[ETH_ADDR_LEN];
    ovs_be16 eth_type; /* always htons(ETH_TYPE_VIRO) */
    ovs_be32 fd_sw;
    ovs_be16 fd_host;
    ovs_be16 eth_next_type;
} __attribute__((packed));
BUILD_ASSERT_DECL(VIRO_ETH_HDR_LEN == sizeof(struct viro_eth_hdr));
