...
void
packet_set_viro(struct dp_packet *packet, ovs_be32 dst_sw, ovs_be16 dst_host,
                ovs_be32 src_sw, ovs_be16 src_host,
                ovs_be32 fd_sw, ovs_be16 fd_host)
{
    struct viro_eth_hdr *vh = dp_packet_l2(packet);
    struct viro_addr *addr;

    if (vh->eth_type == htons(ETH_TYPE_VIRO)) {
        // Chen: addr???
        addr = (struct viro_addr *) vh->eth_dst;
        addr->sw = dst_sw;
        addr->host = dst_host;

        addr = (struct viro_addr *) vh->eth_src;
        addr->sw = src_sw;
        addr->host = src_host;
...
void
viro_push_fd(struct dp_packet *packet, ovs_be32 fd_sw, ovs_be16 fd_host)
{
    struct eth_header *eth;
    struct viro_eth_hdr tmp_vh, *vh;

    eth = dp_packet_data(packet);

    // four 'memcpy' here have type problem???
    memcpy(tmp_vh.eth_dst, eth->eth_dst.ea, ETH_ADDR_LEN);
    memcpy(tmp_vh.eth_src, eth->eth_src.ea, ETH_ADDR_LEN);
    tmp_vh.eth_type = htons(ETH_TYPE_VIRO);
    tmp_vh.fd_sw = fd_sw;
    tmp_vh.fd_host = fd_host;
    tmp_vh.eth_next_type = eth->eth_type;

    vh = dp_packet_push_uninit(packet, VIRO_HDR_LEN);
    memcpy(vh, &tmp_vh, sizeof tmp_vh);

    // Chen: unnecessary now.
    // Because struct dp_packet does not have attr 'l2' now,
    // and 'dp_pakcet_l2' still functions to get packet->l2
    // packet->l2 = packet->data;
}

void
viro_pop_fd(struct dp_packet *packet)
{
    struct viro_eth_hdr *veh = dp_packet_l2(packet);
    if (packet->size_ >= sizeof *veh && veh->eth_type == htons(ETH_TYPE_VIRO)) {
        // Chen: Why tmp???
        struct eth_header tmp;

        memcpy(tmp.eth_dst.ea, veh->eth_dst, ETH_ADDR_LEN);
        memcpy(tmp.eth_src.ea, veh->eth_src, ETH_ADDR_LEN);
        tmp.eth_type = veh->eth_next_type;

        dp_packet_pull(packet, VIRO_HDR_LEN);
        // Chen: unnecessary, check above
        // packet->l2 = (char*)packet->l2 + VIRO_HDR_LEN;

        memcpy(dp_packet_data(packet), &tmp, sizeof tmp);
    }
}
