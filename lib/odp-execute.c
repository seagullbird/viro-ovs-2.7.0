...
odp_execute_set_action(struct dp_packet *packet, const struct nlattr *a)
{
    ...
    case OVS_KEY_ATTR_VIRO:
         viro_key = nl_attr_get_unspec(a, sizeof(struct ovs_key_viro));
         packet_set_viro(packet, viro_key->sw_dst, viro_key->host_dst,
                         viro_key->sw_src, viro_key->host_src,
                         viro_key->sw_fd, viro_key->host_fd);
         break;
                    ...
                    odp_execute_cb dp_execute_action)
{
    ...
    NL_ATTR_FOR_EACH_UNSAFE (a, left, actions, actions_len) {
        ...
        case OVS_ACTION_ATTR_VIRO_PUSH_FD: {
            const struct ovs_action_viro_push_fd *fd = nl_attr_get(a);
            for (i = 0; i < cnt; i++) {
                viro_push_fd(packets[i], fd->fd_sw, fd->fd_host);
            }
            break;
        }
