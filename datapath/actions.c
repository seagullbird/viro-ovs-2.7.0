...
static int set_viro(struct sk_buff *skb, const struct ovs_key_viro *viro_key)
{
	struct viro_eth_vid_hdr *vh;
	int err;

	err = skb_ensure_writable(skb, skb_transport_offset(skb) +
				  sizeof(struct viro_eth_vid_hdr));
	if (unlikely(err))
		return err;

	vh = (struct viro_eth_vid_hdr *) skb->data;
	vh->dst_sw = viro_key->sw_dst;
	vh->dst_host = viro_key->host_dst;
	vh->src_sw = viro_key->sw_src;
	vh->src_host = viro_key->host_src;
	vh->fd_sw = viro_key->sw_fd;
	vh->fd_host = viro_key->host_fd;
...
static int viro_push_fd(struct sk_buff *skb, struct ovs_action_viro_push_fd *action)
{
	struct viro_eth_hdr *ve;

	if (skb_cow_head(skb, VIRO_HDR_LEN) < 0) {
		kfree_skb(skb);
		return -ENOMEM;
	}

	ve = (struct viro_eth_hdr*) skb_push(skb, VIRO_HDR_LEN);



	memmove(skb->data, skb->data + VIRO_HDR_LEN, 2 * ETH_ALEN);
	skb->mac_header -= VIRO_HDR_LEN;

	ve->eth_type = htons(ETH_P_VIRO);
	ve->fd_sw = action->fd_sw;
	ve->fd_host = action->fd_host;

	skb->protocol = htons(ETH_P_VIRO);

	return 0;
}

static int viro_pop_fd(struct sk_buff *skb)
{
	int err;
	struct viro_hdr *vhdr;

	if (unlikely(skb->protocol != htons(ETH_P_VIRO) || skb->len < VIRO_ETH_HDR_LEN))
		return -ENOMEM;

	err = skb_ensure_writable(skb, VIRO_ETH_HDR_LEN);
	if (unlikely(err))
		return err;

	if (skb->ip_summed == CHECKSUM_COMPLETE)
		skb->csum = csum_sub(skb->csum, csum_partial(skb->data + (2 * ETH_ALEN), VIRO_HDR_LEN, 0));

	vhdr = (struct viro_hdr *)(skb->data + ETH_HLEN);

	memmove(skb->data + VIRO_HDR_LEN, skb->data, 2 * ETH_ALEN);
	__skb_pull(skb, VIRO_HDR_LEN);

	skb->protocol = vhdr->eth_next_type;
	skb->mac_header += VIRO_HDR_LEN;
			      ...
			      const struct nlattr *attr, int len)
{
	...
	for (a = attr, rem = len; rem > 0;
		...
		case OVS_ACTION_ATTR_VIRO_PUSH_FD:
			err = viro_push_fd(skb, nla_data(a));
			break;
