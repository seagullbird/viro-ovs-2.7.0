...
static const struct ovs_len_tbl ovs_key_lens[OVS_KEY_ATTR_MAX + 1] = {
	...
	[OVS_KEY_ATTR_VIRO] = sizeof(struct ovs_key_viro),
};
			     ...
			     struct sk_buff *skb)
{
	...
	if (swkey->eth.type == htons(ETH_P_VIRO)) {
		struct ovs_key_viro *viro_key;
		// struct viro_addr *addr;

		nla = nla_reserve(skb, OVS_KEY_ATTR_VIRO, sizeof(*viro_key));
		if (!nla)
			goto nla_put_failure;

		// key_len = SW_FLOW_KEY_OFFSET(viro);
		viro_key = nla_data(nla);

		viro_key->sw_fd = output->viro.fd_sw;
		viro_key->host_fd = output->viro.fd_host;

		viro_key->sw_dst = output->viro.sw_dst;
		viro_key->host_dst = output->viro.host_dst;

		viro_key->sw_src = output->viro.sw_src;
		viro_key->host_src = output->viro.host_src;
...
static int validate_viro(const struct sw_flow_key *flow_key)
{
	return 0;
}
				  ...
				  __be16 eth_type, __be16 vlan_tci, bool log)
{
	...
	nla_for_each_nested(a, attr, rem) {
		...
		static const u32 action_lens[OVS_ACTION_ATTR_MAX + 1] = {
			...
			[OVS_ACTION_ATTR_VIRO_PUSH_FD] = sizeof(struct ovs_action_viro_push_fd),
			[OVS_ACTION_ATTR_VIRO_POP_FD] = 0
		};
