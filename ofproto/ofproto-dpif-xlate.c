...
/* Yi: Viro xlate action. */
static void
xlate_viro_action(struct xlate_ctx *ctx, const struct ofpact *a)
{
    struct ofpact_viro_push_fd *push_fd;

    switch (a->type) {
    case OFPACT_VIRO_PUSH_FD:
        push_fd = ofpact_get_VIRO_PUSH_FD(a);
        ctx->base_flow.viro_fd_sw = push_fd->fd_sw;
        ctx->base_flow.viro_fd_host = push_fd->fd_host;
        break;

    case OFPACT_VIRO_POP_FD:
        ctx->base_flow.viro_fd_sw = htonl(0);
        ctx->base_flow.viro_fd_host = htons(0);
        break;

    case OFPACT_VIRO_SET_VID_SW_FD:
        ctx->base_flow.viro_fd_sw = ofpact_get_VIRO_SET_VID_SW_FD(a)->sw;
        break;

    case OFPACT_VIRO_SET_VID_SW_SRC:
        ctx->base_flow.viro_src_sw = ofpact_get_VIRO_SET_VID_SW_SRC(a)->sw;
        break;

    case OFPACT_VIRO_SET_VID_SW_DST:
        ctx->base_flow.viro_dst_sw = ofpact_get_VIRO_SET_VID_SW_DST(a)->sw;
        break;

    case OFPACT_VIRO_SET_VID_HOST_FD:
        ctx->base_flow.viro_fd_host = ofpact_get_VIRO_SET_VID_HOST_FD(a)->host;
        break;

    case OFPACT_VIRO_SET_VID_HOST_SRC:
        ctx->base_flow.viro_src_host = ofpact_get_VIRO_SET_VID_HOST_SRC(a)->host;
        break;

    case OFPACT_VIRO_SET_VID_HOST_DST:
        ctx->base_flow.viro_dst_host = ofpact_get_VIRO_SET_VID_HOST_DST(a)->host;
        break;
